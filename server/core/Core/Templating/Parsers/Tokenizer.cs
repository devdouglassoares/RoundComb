    ...
    public class Tokenizer
    {
        ...
        public static IEnumerable<TokenPair> Tokenize(string templateString)
        {
            ...
            foreach (Match m in matches)
            {
                ...
                var matchedValue = m.Value.Replace("&nbsp;", " "); // handle html space break;

                if (matchedValue.StartsWith("{{#each"))
                {
                    scopestack.Push(Tuple.Create(matchedValue, m.Index));
                    var token = matchedValue.TrimStart('{').TrimEnd('}').TrimStart('#').Trim();
                ...
                else if (matchedValue.StartsWith("{{/each"))
                {
                    if (matchedValue != "{{/each}}")
                    {
                        var location = HumanizeCharacterLocation(templateString, m.Index, ref lines);
                ...
                else if (matchedValue.StartsWith("{{{") | matchedValue.StartsWith("{{&"))
                {
                ...
                {
                    //unsingle value.
                    var token = matchedValue.TrimStart('{').TrimEnd('}').Trim();
