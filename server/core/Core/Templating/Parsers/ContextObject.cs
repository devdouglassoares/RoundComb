        ...
        private ContextObject GetContextForPath(Queue<String> elements)
        {
            ...
            if (elements.Any())
            {
                ...
                else
                {
                    ...
                    var value = ObjectCopyExtensions.TryGetValue(Value, element);

                    if (value != null && value.GetType().Name.IndexOf("DateTime") > -1)
                    {
                        innerContext.Value = value.ToString("MM/dd/yyyy HH:mm:ss");
                    }
                    else
